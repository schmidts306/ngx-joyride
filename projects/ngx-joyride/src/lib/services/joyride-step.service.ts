...
import {
    JoyrideStepDoesNotExist,
    JoyrideStepOutOfRange,
} from '../models/joyride-error.class';
...
import {
    ...
    StepActionType,
} from './joyride-steps-container.service';
import { LoggerService } from './logger.service';
import { StepDrawerService } from './step-drawer.service';

const SCROLLBAR_SIZE = 20;

export const DISTANCE_FROM_TARGET = 30;
...
export class JoyrideStepService implements IJoyrideStepService {
    ...
    private stepsObserver: ReplaySubject<JoyrideStepInfo> =
        new ReplaySubject<JoyrideStepInfo>();
    ...
    private initViewportPositions() {
        ...
        this.winBottomPosition =
            this.DOMService.getNativeWindow().innerHeight - SCROLLBAR_SIZE;
    }
    ...
    private subscribeToScrollEvents() {
        ...
        this.eventListener.scrollEvent.subscribe((scroll) => {
            ...
            this.winBottomPosition =
                this.winTopPosition +
                this.DOMService.getNativeWindow().innerHeight -
                SCROLLBAR_SIZE;
            if (this.currentStep)
    ...
    private drawStep(step: JoyrideStep) {
        step.position =
            step.position === NO_POSITION
                ? this.optionsService.getStepDefaultPosition()
                : step.position;
    ...
    private subscribeToStepsUpdates() {
        this.stepsContainerService.stepHasBeenModified.subscribe(
            (updatedStep) => {
                if (
                    this.currentStep &&
                    this.currentStep.name === updatedStep.name
                ) {
    ...
    private notifyStepClicked(actionType: StepActionType) {
        let stepInfo: JoyrideStepInfo = {
            number: this.stepsContainerService.getStepNumber(
    ...
    private scrollWhenTargetOrStepAreHiddenBottom() {
        ...
        if (totalTargetBottom > this.winBottomPosition) {
            this.DOMService.getNativeWindow().scrollBy(
                0,
                totalTargetBottom - this.winBottomPosition
            );
        }
    }
    ...
    private scrollWhenTargetOrStepAreHiddenTop() {
        ...
        if (totalTargetTop < this.winTopPosition) {
            this.DOMService.getNativeWindow().scrollBy(
                0,
                totalTargetTop - this.winTopPosition
            );
        }
    }

    private getMaxTargetAndStepBottomPosition(): number {
        let targetAbsoluteTop = this.documentService.getElementAbsoluteTop(
        ...
        if (this.currentStep.position === 'top') {
            ...
            this.currentStep.position === 'right' ||
            this.currentStep.position === 'left'
        ) {
            return Math.max(
                targetAbsoluteTop + this.currentStep.stepInstance.targetHeight,
                targetAbsoluteTop +
                    this.currentStep.stepInstance.targetHeight / 2 +
                    this.currentStep.stepInstance.stepHeight / 2
            );
        }
    }
    ...
    private scrollIfElementBeyondOtherElements() {
        if (this.isElementBeyondOthers() === 2) {
            this.documentService.scrollToTheTop(
                this.currentStep.targetViewContainer.element
            );
        }
        if (this.isElementBeyondOthers() === 2) {
            this.documentService.scrollToTheBottom(
                this.currentStep.targetViewContainer.element
            );
        }
        if (
            this.isElementBeyondOthers() === 1 &&
            this.documentService.isParentScrollable(
                this.currentStep.targetViewContainer.element
            )
        ) {
